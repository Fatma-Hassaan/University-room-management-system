...
namespace Project.Models
{
    public class DB
    {
        ...
        public string GetUserType(string Email)
        {
            ...
            {
                string query = "SELECT UserType FROM [User] WHERE Email = @Email UNION SELECT UserType FROM [Admin] WHERE Email = @Email";
                using (SqlCommand cmd = new SqlCommand(query, con))
                {
                    ...
                    {
                        // Log the exception
                        Console.WriteLine($"Error in GetUserType: {ex.Message}");
                        throw;
                    }
                }
            }
        ...
        public bool CheckPassword(int ID, string Password)
        {
            using (var con = CreateConnection())
            {
                string query = "SELECT [Password] FROM [User] WHERE UserID = @ID UNION SELECT [Password] FROM [Admin] WHERE ID = @ID";
                using (SqlCommand cmd = new SqlCommand(query, con))
                {
        ...
        public DataTable RetrieveAllRooms()
        {
        ...
        public DataTable RoomSearchByCourse(string CourseCode, string Type)
        {
            DataTable dt = new DataTable();

            string query = "\r\nSELECT \r\n    C.Code AS CourseCode,\r\n    CASE \r\n        WHEN @Type = 'Lecture' THEN 'Lecture'\r\n        WHEN @Type = 'Tutorial' THEN 'Tutorial'\r\n        WHEN @Type = '' AND R.ID = C.LectureRoom THEN 'Lecture'\r\n        WHEN @Type = '' AND R.ID = C.TutorialRoom THEN 'Tutorial'\r\n        ELSE NULL\r\n    END AS CourseType,\r\n    CASE \r\n        WHEN @Type = 'Lecture' THEN C.LectureDay\r\n        WHEN @Type = 'Tutorial' THEN C.TutorialDay\r\n        WHEN @Type = '' AND R.ID = C.LectureRoom THEN C.LectureDay\r\n        WHEN @Type = '' AND R.ID = C.TutorialRoom THEN C.TutorialDay\r\n        ELSE NULL\r\n    END AS TimeSlotDay,\r\n    CASE \r\n        WHEN @Type = 'Lecture' THEN CONVERT(VARCHAR(5), C.LectureHour, 108)\r\n        WHEN @Type = 'Tutorial' THEN CONVERT(VARCHAR(5), C.TutorialHour, 108)\r\n        WHEN @Type = '' AND R.ID = C.LectureRoom THEN CONVERT(VARCHAR(5), C.LectureHour, 108)\r\n        WHEN @Type = '' AND R.ID = C.TutorialRoom THEN CONVERT(VARCHAR(5), C.TutorialHour, 108)\r\n        ELSE NULL\r\n    END AS TimeSlotHour,\r\n\r\n    R.*\r\nFROM Room R\r\nJOIN Course C\r\n    ON (\r\n        (@Type = 'Lecture' AND R.ID = C.LectureRoom) OR\r\n        (@Type = 'Tutorial' AND R.ID = C.TutorialRoom) OR\r\n        (@Type = '' AND (R.ID = C.LectureRoom OR R.ID = C.TutorialRoom))\r\n    )\r\nWHERE C.Code = @CourseCode";
            SqlCommand cmd = new SqlCommand(query, con);
        ...
        public DataTable RoomSearchForOffice(string StaffName)
        {
            DataTable dt = new DataTable();

            string query = "SELECT\tCOALESCE(U_P.Name, U_T.Name) AS StaffName, R.*\r\nFROM Room R\r\nLEFT JOIN Professor P ON P.OfficeRoom = R.ID\r\nLEFT JOIN TA T ON T.OfficeRoom = R.ID\r\nLEFT JOIN [User] U_P ON U_P.UserID = P.ID\r\nLEFT JOIN [User] U_T ON U_T.UserID = T.ID\r\nWHERE (U_P.[Name] LIKE '%' + @StaffName + '%' OR U_T.[Name] LIKE '%' + @StaffName + '%')";
            SqlCommand cmd = new SqlCommand(query, con);
        ...
        public DataTable RoomSearchByLocation(string Building, int? Floor, string Zone)
        {
            DataTable dt = new DataTable();
        ...
        public DataTable SearchUsers(string UserType, string Name)
        {
            ...
            else if (Name == "") query = "Select UserID, UserType, [Name], Email From [User] Where [UserType] = @UserType";
            else if (Name != "" && UserType != "") query = "Select UserID, UserType, [Name], Email From [User] Where [UserType] = @UserType And [Name] like '%' + @Name + '%'";
        ...
        public bool DoesProfessorExist(int ProfessorId)
        {
            bool exist;
            string query = "SELECT 1 FROM Professor WHERE ID = @ProfessorId";
            SqlCommand cmd = new SqlCommand(query, con);
            cmd.Parameters.AddWithValue("@ProfessorId", ProfessorId);
            try
            {
                con.Open();
                exist = (bool)cmd.ExecuteScalar();
            }
            catch (Exception ex)
            {
        ...
        public bool DoesTAExist(int TaId)
        {
            bool exist;
            string query = "SELECT 1 FROM TA WHERE ID = @TaId";
            SqlCommand cmd = new SqlCommand(query, con);
            cmd.Parameters.AddWithValue("@TaId", TaId);
            try
            {
        ...
        public bool DoesJTAExist(int JtaId)
        {
            bool exist;
            string query = "SELECT 1 FROM JTA WHERE Student_ID = @JtaId";
            SqlCommand cmd = new SqlCommand(query, con);
            cmd.Parameters.AddWithValue("@JtaId", JtaId);
            try
            {
        ...
        {
            bool exist;
            string query = "SELECT 1 FROM Student WHERE ID = @StudentId";
            SqlCommand cmd = new SqlCommand(query, con);
            cmd.Parameters.AddWithValue("@StudentId", StudentId);
            try
            {
        ...
        {
            bool exist;
            string query = "SELECT 1 FROM Room WHERE ID = @RoomId";
            SqlCommand cmd = new SqlCommand(query, con);
